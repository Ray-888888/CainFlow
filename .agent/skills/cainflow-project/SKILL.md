...
1. 在动手前先给需求分类。
   ...
   - 版本更新检查、更新通知、更新模块显隐与直接下载更新 -> `js/core/constants.js`、`index.js`、`js/features/update/update-manager.js`、`js/features/settings/settings-controller.js`、`backend/routes/update_routes.py`、`backend/services/update_service.py`、`backend/config.py`
   - 节点定义或序列化 -> `js/nodes/*`
...
- 如果要做“锁定 API 供应商”这类全局策略开关，默认值与开关常量放 `js/core/constants.js`，设置页里的可见性/只读态放 `js/features/settings/settings-controller.js`，配置导入与会话恢复的绕过保护同步检查 `js/features/ui/ui-controller.js`、`js/features/persistence/project-io.js`。
...
- `TextSplit` 节点有 1 个文本输入口，不再提供节点内部长文本输入框；输入文本来自上游连接或旧工作流恢复数据。输出口数量由节点内“输出数量”步进控件控制：大于 0 时按配置数量生成端口，等于 0 时根据分割结果自动生成端口，并在控件底部提示用户；输入框只允许数字，左右按钮负责增减。端口模板、输出数量控件、节点内预览与动态重建放 `js/nodes/node-view-factory.js`、`js/nodes/node-dom-bindings.js`，执行输出分发与动态端口取值放 `js/features/execution/execution-core.js`，保存/复制时同步 `delimiter`、`outputCount`、`removeEmptyLines`、`previewEnabled`、`parts` 等字段到 `js/nodes/node-serializer.js` 与 `js/features/ui/clipboard-controller.js`。旧工作流没有 `outputCount` 时，应继续从已有 `parts` 或分割结果推导端口数量，避免旧连线失效。开启节点内预览后，预览区应在节点内部 flex/滚动展示，不得把完整预览内容计入节点最小高度或阻止用户手动缩小节点。
...
- `TextChat` 回复框结构由 `js/nodes/node-view-factory.js` 与 `css/legacy.css` 维护：复制按钮是小按钮，放在回复文本框内部左上角；回复内容在框内滚动展示，不得强制完整撑开节点。回复内容和 TextSplit 预览这类可滚动结果区的最小高度测量放 `js/nodes/node-lifecycle.js` / `js/nodes/node-dom-bindings.js`，测量时应忽略实际长内容高度，只保留可用的最小容器尺寸；包含 `.chat-response-area`、`.text-display-box`、`.text-split-preview` 的容器不要再用 `offsetHeight` / `scrollHeight` 作为强制高度兜底，否则长回复会在 resize 时重新撑开节点。
...
- 节点最小尺寸测量统一收口在 `js/nodes/node-lifecycle.js`。测量时不能只靠手写递归估算 DOM 子元素高度；要结合浏览器真实布局的 `offsetHeight` / `scrollHeight` 做高度兜底，尤其是窄宽度换行、grid/flex 重排、预览区、按钮组、原生控件和文本框。但对 TextChat 回复区、TextSplit 预览区、文本显示框这类可滚动长内容区，应跳过完整内容 `scrollHeight` 兜底，只按稳定容器最小高度计算。
...
- 新增或修改节点内部固定 UI 块时，要把该块纳入共享测量链：可视媒体区加入 media-like 测量选择器，滚动结果区使用稳定 `minHeight`，长内容区域只测容器可用高度，不按完整文本撑开。不要用局部 CSS `overflow: hidden` 或临时增大默认高度掩盖测量缺口；如果节点需要“新建时较高、手动缩小时较低”的体验，应在节点类型定义里拆分 `defaultHeight` 与 `minHeight`，不要继续把 `defaultHeight` 当作 resize 的硬下限。
...
- 运行中单节点取消责任链固定为：浮动取消按钮结构放 `js/nodes/node-view-factory.js`，长按 2 秒与拖动阈值判定放 `js/nodes/node-dom-bindings.js`，取消入口由 `index.js` 注入到节点绑定，执行器在 `js/features/execution/workflow-runner.js` 用 `state.runningNodeCancelHandlers` 与单节点 `AbortController` 只中止该节点并把本次计划内下游标记为跳过；不要 abort 整个运行 session，也不要影响其他正在运行的无关节点。运行态锁禁用节点内控件时必须排除 `.node-run-cancel-btn`，否则用户无法触发取消；按钮样式放 `css/components/nodes.css`，浅色主题覆盖放 `css/themes.css`，操作说明同步补 `js/features/help/help-panel.js`。
...
- `js/core/constants.js` 里的 `API_PROVIDERS_LOCKED` 用于控制供应商是否允许新增、删除或修改默认 URL。开启后，设置页应隐藏“添加供应商”和供应商删除入口，供应商 `endpoint` 只读；模型管理保持可用，但导入配置和会话恢复不能借机替换被锁定的默认供应商 URL。
...
- 涉及运行中节点取消时，要区分“取消一个节点及其下游”和“停止整个工作流”：顶部停止按钮继续走 `state.runAbortControllers` / 全局 session abort；节点浮动取消只调用 `state.runningNodeCancelHandlers.get(nodeId)`，并由当前 workflow session 负责下游跳过、清理运行态和保留其他并发节点。
...
- 更新能力的全局关闭参数使用 `js/core/constants.js` 的 `AUTO_UPDATE_CHECK_DISABLED`。为 `true` 时，`index.js` 注入 `autoUpdateCheckDisabled` 给 `js/features/update/update-manager.js`，启动期 `scheduleAutoUpdateCheck()` 不再倒计时或请求 GitHub；同时 `js/features/settings/settings-controller.js` 不渲染“系统版本与更新”卡片。手动更新入口若需要保留或隐藏，必须同步检查这条常量链，不要只改启动流程或只改设置页。
