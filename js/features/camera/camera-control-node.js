...
function createThreeQuarterLabel(direction) {
    return direction === 'left'
        ? 'Left three-quarter view'
        : 'Right three-quarter view';
}

function createRearThreeQuarterLabel(direction) {
    return direction === 'left'
...
export function generateCameraPrompt(cameraData = {}) {
    ...
    const horizontalDirection = getSubjectHorizontalDirection(yaw);
    const segments = [];

    if (pitch >= 60) {
        segments.push("Bird's-eye view", 'Top-down perspective');
    } else if (pitch >= 32) {
        segments.push('High angle shot', 'Elevated perspective');
    } else if (pitch >= 12) {
        segments.push('Slight high angle');
    } else if (pitch <= -38) {
        segments.push("Low angle shot", "Worm's-eye view");
    } else if (pitch <= -16) {
        segments.push('Low angle shot');
    } else if (pitch <= -6) {
        segments.push('Subtle low angle');
    } else {
        segments.push('Eye-level shot');
    }

    if (absYaw <= 18) {
        segments.push('Front view', 'Straight-on composition');
    } else if (absYaw <= 68) {
        segments.push(createThreeQuarterLabel(horizontalDirection));
    } else if (absYaw <= 112) {
        segments.push(horizontalDirection === 'left' ? 'Left profile view' : 'Right profile view', 'Side shot');
    } else if (absYaw <= 162) {
        segments.push(createRearThreeQuarterLabel(horizontalDirection));
    } else {
        segments.push('Rear view');
    }

    if (distance <= 2.4) {
        segments.push('Extreme close-up shot');
    } else if (distance <= 4) {
        segments.push('Close-up shot');
    } else if (distance <= 5.8) {
        segments.push('Medium close-up shot');
    } else if (distance <= 8.2) {
        segments.push('Medium shot');
    } else if (distance <= 12) {
        segments.push('Full shot');
    } else {
        segments.push('Long shot', 'Wide framing');
    }

    if (fov < 28) {
        segments.push('Super-telephoto lens compression');
    } else if (fov < 42) {
        segments.push('Telephoto lens', 'Compressed perspective');
    } else if (fov < 65) {
        segments.push('Standard lens perspective');
    } else if (fov < 86) {
        segments.push('Wide-angle lens');
    } else if (fov < 108) {
        segments.push('Ultra-wide angle');
    } else {
        segments.push('Ultra-wide angle', 'Fisheye lens effect');
    }

    const absRoll = Math.abs(roll);
    if (absRoll >= 16) {
        segments.push('Strong Dutch angle', 'Dramatic camera tilt');
    } else if (absRoll >= 6) {
        segments.push('Dutch angle', 'Slight camera tilt');
    }
